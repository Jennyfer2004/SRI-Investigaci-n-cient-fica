...
if "user_id" not in st.session_state:
    st.session_state.user_id = str(hash(st.experimental_user.email))  # Identificador único del usuario
...
if True:
    ...
    # Mostrar las recomendaciones como botones
    cols = st.columns(2)
    for i, pregunta in enumerate(recomendaciones[:4]):  # Mostrar 4 recomendaciones
        with cols[i % 2]:
...
# Si se usó una pregunta recomendada
...
if user_input:
        ...
        if metadatos:
            ...
            for paper in metadatos:
                with st.expander(f"**{paper['titulo']}**"):
                    ...
                    # Mostrar DOI y enlaces como hipervínculos
    ...
    # ✅ Volver a mostrar recomendaciones después de la respuesta
